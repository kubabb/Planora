  ...
  const sections = [
    {
      ...
      title: 'Overview',
      body:
        'Planora is a developer tool — CLI, VS Code extension, and local React web app — that automates project planning. It generates structured Markdown plans, interactive mind maps (markmap), and Mermaid architecture diagrams. Runs with its own AI agent. No Hermes required.',
    },
    {
      id: 'quickstart',
      title: 'Quick Start',
      body:
        'Three commands from zero to a fully planned project. OpenRouter is the recommended default provider for first-run setup.',
      code: [
        'npm install -g planora',
        'planora config                        # wizard: API key + provider + model',
        'planora init --name "my-app" --stack "react,node,postgres"',
        'planora plan --ai                     # AI generates all planning docs',
        'planora web                           # opens localhost:4173 dashboard',
      ],
    },
    {
      id: 'philosophy',
      title: 'Philosophy',
      body:
        'Markdown is the single source of truth. Every artifact is a plain .md file — git-friendly, human-readable, no proprietary lock-in. CLI, VS Code, and web app are different views on the same data. Local-first — plans live on disk, localhost is the dashboard.',
      items: [
        ['Markdown-first', 'All plans, mind maps, and diagrams are .md files'],
        ['Zero friction', 'Three commands: config → init → plan → web'],
        ['Local-first', 'No cloud required. SQLite on disk. localhost:4173'],
        ['Own agent', 'Planora handles prompts, tools, and workflows. Hermes is optional.'],
        ['Security', 'Configs are 0600. Secrets never touch disk unencrypted. Every push scanned.'],
      ],
    },
    {
      id: 'architecture',
      title: 'Architecture',
      body:
        'The monorepo is split into five packages: shared core logic, three execution surfaces (CLI, web, VS Code), and an agent runtime. TypeScript strict, Node.js 20+, npm workspaces.',
      code: [
        '@planora/core         models, AI client, generators, storage, memory, analyzers, utils',
        '@planora/cli          10 commands: init, plan, config, analyze, roadmap, mindmap, code, review, agent, web',
        '@planora/web          React 18 + Vite 5 dashboard on localhost:4173',
        '@planora/runner       Agent engine: prompts, 8 tools, 3 workflows (plan/code/review)',
        '@planora/vscode-ext    Command Palette + Webview (planned)',
      ],
    },
    {
      id: 'core-modules',
      title: 'Core Modules',
      body:
        '@planora/core contains 8 modules covering the entire product surface — from AI communication to file generation and repo analysis.',
      items: [
        ['ai/', 'AiClient — fetch-based LLM client. 5 providers: OpenRouter, OpenAI, Ollama, OpenCode, custom compatible. Retry with exponential backoff. Structured output via Zod.'],
        ['config/', '~/.planora/config.json (chmod 600). Interactive wizard. Provider/model selection. Connection test. API key masking.'],
        ['models/', 'Project, User, PlanFile, AgentRun. SQLite-ready schemas. AgentRun replaces old HermesRun naming.'],
        ['generators/', '6 static generators producing .md files: project-plan, roadmap, mindmap, architecture, agent-setup, planora-json. Works without AI.'],
        ['storage/', 'SQLite via better-sqlite3 (WAL mode). Tables: users, projects, agent_runs. Supabase adapter for future cloud sync.'],
        ['memory/', 'Qdrant Cloud vector memory (fetch-based, zero deps). 4 collections: project_plans, code_snippets, decisions, agent_memory. Cosine distance, 1024-dim vectors.'],
        ['analyzers/', 'Repo analyzer detects language, framework, runtime, quality score (0-100). Stack recommender with 6 keyword patterns. Zero AI — pure pattern matching.'],
        ['utils/', 'Mermaid builder: flowcharts, sequence diagrams, gantt charts. Programmatic string output, zero dependencies.'],
      ],
    },
    {
      id: 'ai-agent',
      title: 'AI Agent',
      body:
        'Planora has its own AI agent. Users provide only an API key — Planora handles prompts, tool calling, retries, and session management. No Hermes installation needed.',
      items: [
        ['AiClient', 'Direct LLM client: generate(), generateWithTools(), generateStructured(), generateStream(), testConnection(). All fetch-based, no SDK dependency.'],
        ['Agent Engine', 'runner/src/agent.ts — think → act → observe loop. Session management, tool registry, workflow orchestration. Configurable timeout and retry.'],
        ['Tools', '8 function-calling tools: file_read, file_write, file_list, search, shell, web_search (SearXNG), memory_store, memory_search (Qdrant).'],
        ['Workflows', '3 built-in: plan (generates all .md files), code (implements features), review (code quality check). Each ends with SQLite + Qdrant save.'],
        ['Providers', 'OpenRouter (recommended), OpenAI direct, Ollama (local), OpenCode, custom OpenAI-compatible. Auto-detects OpenRouter keys (sk-or-v1- prefix).'],
        ['Prompts', '7 system prompts: base, planner, coder, reviewer — in Polish and English. Tool-use instructions built in.'],
      ],
    },
    {
      id: 'cli',
      title: 'CLI Commands',
      body:
        'The CLI is the primary interface. 10 commands cover the full lifecycle: configuration, initialization, planning, analysis, and agent operations.',
      items: [
        ['planora config', 'Interactive AI setup wizard. Also: --show, --test, --set-key, --set-model.'],
        ['planora init', 'Initialize workspace. Creates .planora/ directory with SQLite DB.'],
        ['planora plan', 'Generate all planning docs. --ai flag invokes the agent.'],
        ['planora analyze', 'Analyze existing repo: detects stack, structure, quality score.'],
        ['planora roadmap', 'Generate ROADMAP.md with phased delivery plan.'],
        ['planora mindmap', 'Generate MINDMAP.md for markmap rendering.'],
        ['planora code', 'AI implements a feature. Agent writes the code.'],
        ['planora review', 'AI code review with quality suggestions.'],
        ['planora agent', 'Agent status, run history (SQLite), token usage.'],
        ['planora web', 'Launch React dashboard on localhost:4173.'],
      ],
    },
    {
      id: 'generated-files',
      ...
      body:
        'Every planora plan --ai produces 6 files in the .planora/ directory. All are plain Markdown or JSON — readable, version-controllable, and editable.',
      items: [
        ['PROJECT_PLAN.md', 'Overview, goals, MVP definition, tech stack, milestones'],
        ['ROADMAP.md', 'Phased delivery timeline with sequencing rationale'],
        ['MINDMAP.md', 'Hierarchical outline rendered as interactive mind map (markmap)'],
        ['ARCHITECTURE.md', 'Mermaid diagrams: system context, data flow, component topology'],
        ['AGENT_SETUP.md', 'Agent configuration: provider, model, workflow notes'],
        ['planora.json', 'Machine-readable project metadata for the dashboard'],
      ],
    },
    {
      id: 'web-app',
      title: 'Web App',
      body:
        'Local React dashboard at localhost:4173. Built with Vite 5, React 18, Tailwind CSS. Renders mind maps via markmap and architecture diagrams via Mermaid.js.',
      items: [
        ['Dashboard', 'List of all projects with status and metadata'],
        ['Project View', 'Per-project details: overview, roadmap, generated files'],
        ['Mind Map View', 'Full-screen interactive mind map from MINDMAP.md'],
        ['Graphs View', 'Mermaid diagram rendering from ARCHITECTURE.md'],
        ['Agent View', 'Agent status, model info, run history, token consumption'],
      ],
    },
    {
      id: 'memory',
      ...
      body:
        'Dual storage: SQLite for structured data (users, projects, runs), Qdrant Cloud for semantic vector memory. Both local-first and zero-config for end users.',
      items: [
        ['SQLite', 'better-sqlite3, WAL mode. Tables: users, projects, agent_runs. Single file per project in .planora/.'],
        ['Qdrant Cloud', '4 vector collections, 1024-dim, cosine distance. Stores: plans, code snippets, decisions, agent memory across sessions.'],
        ['Config security', '~/.planora/config.json and qdrant-config.json are chmod 600. Never committed. API key masked in all outputs.'],
      ],
    },
    {
      ...
      body:
        'Planora is under active development. The core, AI client, agent engine, and CLI are functional. Web app and VS Code extension are in progress.',
      items: [
        ['M1 ✅', 'Monorepo skeleton — 5 packages, build passes, CI configured'],
        ['M2 ✅', 'Core — 40 files: AiClient, config system, generators, SQLite, Qdrant, analyzers'],
        ['M5 ✅', 'CLI + Agent Engine — 10 commands, 8 tools, 3 workflows, 25 unit tests'],
        ['M4 🔲', 'Web app — dashboard, project views, mind map and Mermaid rendering'],
        ['M6 🔲', 'VS Code extension — Command Palette + Webview'],
        ['M7 🔲', 'Hermes optional orchestrator — multi-agent workflows for power users'],
      ],
    },
    {
      id: 'security',
      title: 'Security',
      body:
        'Security is built into the pipeline. Secrets are never exposed in logs, configs are permission-locked, and every push is scanned for leaks.',
      items: [
        ['Config', '~/.planora/config.json: chmod 600. API key masked (sk-...****) in all output.'],
        ['Pre-push', 'npm audit, gitleaks, .env staging check. Blocks push on leaks.'],
        ['CI/CD', 'Step-based guard pattern. Dependency audit + CodeQL + Semgrep on every push.'],
        ['No telemetry', 'Planora never phones home. No analytics, no usage tracking, no cloud requirement.'],
      ],
    },
  ];
